...
public class RoadNetworkTests
{
    ...
    {
        var grid = new CityGrid(10, 10);
        // 2x2 Residential block at (2,2)-(3,3), road only on the left edge touching (2,2)
        ...
        // Only the tile directly adjacent to the road gets road access
        Assert.That(grid.GetTile(2, 2).HasRoadAccess, Is.True, "Tile (2,2) is directly adjacent to road — should have access");
        Assert.That(grid.GetTile(3, 2).HasRoadAccess, Is.False, "Tile (3,2) is not adjacent to road — no direct access");
        Assert.That(grid.GetTile(2, 3).HasRoadAccess, Is.False, "Tile (2,3) is not adjacent to road — no direct access");
        Assert.That(grid.GetTile(3, 3).HasRoadAccess, Is.False, "Tile (3,3) is not adjacent to road — no direct access");
        Assert.That(_roads.AccessibleZoneCount, Is.EqualTo(1));
    }
}

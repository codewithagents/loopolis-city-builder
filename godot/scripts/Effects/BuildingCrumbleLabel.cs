...
/// <summary>
/// Floating label that rises and fades out above a degraded (crumbled) building.
/// Uses an orange-red color and a slightly slower animation than BuildingBirthLabel (2s vs 1.5s).
...
/// </summary>
public partial class BuildingCrumbleLabel : Node2D
{
    private const float Duration   = 2.0f;
    private const float RisePixels = TilemapRenderer.TileSize * 2f;

    // Orange-red crumble color
    private static readonly Color TextColor = new Color(1f, 0.4f, 0.1f, 1f);
    ...
    {
        _text     = text;
        _startPos = worldCenter - new Vector2(0f, TilemapRenderer.TileSize * 0.5f);
        Position  = _startPos;
        _elapsed  = 0f;
        _running  = true;
    }
